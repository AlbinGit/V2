...
- (void)initData {
    _memoDetails = [NSMutableArray array];
    NSArray *dataArray = [self memoRecordReadTheData];
    for (NSDictionary *dic in dataArray) {
        JWMemoDetail *model = [JWMemoDetail new];
        model.title = dic[@"title"];
        model.detail = dic[@"detail"];
        model.createTime = dic[@"createTime"];
        [_memoDetails addObject:model];
    }
}
...
-(NSArray *)memoRecordReadTheData{
    NSInteger accountId = [SGAccountManager sharedManager].currentAccount.accountId;
    NSString *plistName = [NSString stringWithFormat:@"memoRecord_%ld.plist",(long)accountId];
    NSString * filePath = [[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) firstObject] stringByAppendingPathComponent:plistName];
...
- (void)saveData {
    ...
    NSString *plistName = [NSString stringWithFormat:@"memoRecord_%ld.plist",(long)accountId];
    NSString * filePath = [[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) firstObject] stringByAppendingPathComponent:plistName];
    NSMutableArray * historicalRecordArr = [NSMutableArray array];
    for (JWMemoDetail *model in _memoDetails) {
        NSDictionary *dic = @{@"title":model.title,
                              @"detail":model.detail,
                              @"createTime":model.createTime
                              };
        [historicalRecordArr addObject:dic];
    }
